...
def _make_disc(cover_bytes: bytes | None, size: int, hole_ratio: float = 0.020) -> Image.Image:
    ...
    # 盘毂：液态玻璃效果（强模糊基底 + 玻璃提亮 + 月牙高光 + 边缘折射）
    hub_r = S * 0.095
    hub_d = int(hub_r * 2)
    c0 = int(cx - hub_r)
    # 折射放大：取略小的中心区域放大，模拟凸透镜效果
    mag = int(hub_d * 0.78)
    m0 = int(cx - mag / 2)
    ...
    hl = Image.new("RGBA", (hub_d, hub_d), (0, 0, 0, 0))
    ImageDraw.Draw(hl).ellipse(
        (int(hub_d * 0.16), int(hub_d * 0.05), int(hub_d * 0.84), int(hub_d * 0.48)),
    ...
    ImageDraw.Draw(dk).ellipse(
        (int(hub_d * 0.18), int(hub_d * 0.55), int(hub_d * 0.82), int(hub_d * 0.98)),
        fill=(20, 24, 32, 42),
    )
